...
export async function initDb(): Promise<void> {
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_events_device_id ON events (device_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_events_user_id ON events (user_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_identity_map_user_id ON identity_map (user_id)`);
}
